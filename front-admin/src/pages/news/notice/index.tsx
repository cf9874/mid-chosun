...
const NewsNoticeManagementPage = () => {
  const { Column } = Table
  ...
  const onEdit = async () => {
    ...
    const editRes = await apiManager.newsApi.editNews({
      seq: record.seq,
      type: "NOTICE",
      title: editTitle,
      content: body,
    })
    console.log(9393, editRes)
    if (apiUtil.isErrorResponse(editRes)) {
      alert(editRes.message ? editRes.message : "서버에 문제가 발생했습니다.")
      return
    }
    const response = await apiManager.newsApi.getList({
      page: 1,
  ...
  return (
    <LayoutMain>
      ...
      >
        ...
        <Row className={styles.html_box} dangerouslySetInnerHTML={{ __html: record?.content ?? "" }}></Row>
      </BasicModal>
      <BasicModal
        title="공지사항 수정"
...
export default AuthHoc(NewsNoticeManagementPage)
